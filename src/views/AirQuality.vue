...
<template>
  <div class="airQuality">
    ...
    <h4 style="margin-left: 10px; padding-left: 10px;">
      Current Air Quality for {{ airQuality.stations[0].placeName }}
      {{ airQuality.stations[0].postalCode }},
      {{ airQuality.stations[0].state }}
    </h4>
    <p style="margin-left: 10px; padding-left: 10px;">
      Last updated at {{ airQuality.stations[0].updatedAt }}
    </p>
    ...
    <div class="columns medium-6">
      ...
      <ul>
        <li v-for="(item, index) in airQuality.stations" v-bind:key="index">
          ...
          <h4>Air Quality</h4>
          <p>Air Quality Index: {{ item.AQI }}</p>
          <p>Particulant Matter: {{ item.aqiInfo.pollutant }}</p>
          <p>PM Concentration: {{ item.aqiInfo.concentration }}</p>
          <p>AQI Category: {{ item.aqiInfo.category }}</p>
        </li>
      </ul>
    </div>
...
export default {
  ...
  computed: {
    ...
    parameters() {
      return Object.values(this.airQuality).map(({ stations }) => ({
        stations,
      }));
    },
  },
  ...
  mounted() {
    var optionsair = {
      method: "GET",
      url: "https://api.ambeedata.com/latest/by-postal-code",
      params: { postalCode: this.$route.query.zip, countryCode: "US" }, //postal code 90250
      headers: { "x-api-key": apikey, "Content-type": "application/json" },
    };
...
//const apikey = 'N4rk05Lsxs6pG7FfzqYl18newiD5aJZB6zReT8Pn'; //derya apikey
const apikey = "oLCKWMNsb84Z7k1RHAoP1TjSOniPFvy61mJfUt23"; //deniz apikey
