    ...
    <div id="statesSelection">
      <label>
        ...
        <input
          type="text"
          v-model="selected"
          v-on:input="fetchAPIData"
          list="state"
        />
        <datalist id="state">
          <select @change="fetchAPIData()" class="form-control">
            <option
              v-for="(stateOpt, indx) in states"
              :value="stateOpt"
              v-bind:key="indx"
            >
...
export default {
  ...
  computed: {
    stateSelected() {
      //console.log("Changed state:" + this.selected);
  ...
  methods: {
    ...
    fetchAPIData() {
      ...
      this.responseAvailable = false;
      fetch(
        "https://api.weather.gov/alerts?status=actual&message_type=alert&region_type=land&severity=severe",
        {
          method: "GET",
          headers: {},
        }
        ...
        .then((response) => {
          ...
          for (var i = 0; i < response.features.length; i++) {
            if (
              response.features[i].properties.senderName.includes(this.selected)
            ) {
              //was selectedState
              result[c] = JSON.stringify(
                response.features[i].properties.headline
              );
              c++;
            }
          }
...
<style scoped>
.alertsPage {
  font-family: Verdana, Calibri Light, Consolas, Papyrus, Avenir, Helvetica,
    Arial, sans-serif;
}
...
.alertsLi {
  ...
  font-size: 20px;
  font-weight: 600;
  font-family: Calibri Light, Verdana, Consolas, Papyrus, Avenir, Helvetica,
    Arial, sans-serif;
}
</style>
